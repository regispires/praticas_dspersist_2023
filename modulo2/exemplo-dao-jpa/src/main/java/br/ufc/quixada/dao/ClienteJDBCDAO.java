...
public class ClienteJDBCDAO implements ClienteDAO {
	...
	public void save(Cliente entity) {
		...
		try {
			...
			String insert_sql = "insert into cliente (cpf, nome, fone, renda) values (?, ?, ?, ?)";
			String update_sql = "update cliente set cpf = ?, nome = ?, fone = ?, renda = ? where id = ?";
	...
	public Cliente find(int id) {
		...
		try {
			con = ConnectionFactory.getConnection();
			String sql = "select id, cpf, nome, fone, renda from cliente where id = ?";
			PreparedStatement pst = con.prepareStatement(sql);
			pst.setInt(1, id);
	...
	public Cliente findByCpf(String cpf) {
		...
		try {
			con = ConnectionFactory.getConnection();
			PreparedStatement pst;
			String sql = "select id, cpf, nome, fone, renda from cliente where cpf = ?";
			pst = con.prepareStatement(sql);
			pst.setString(1, cpf);
			ResultSet rs = pst.executeQuery();
			if (rs.next()) {
	...
	public List<Cliente> findByNome(String str) {
		...
		try {
			con = ConnectionFactory.getConnection();
			PreparedStatement pst;
			String sql = "select id, cpf, nome, fone, renda from cliente where upper(nome) like ?";
			pst = con.prepareStatement(sql);
